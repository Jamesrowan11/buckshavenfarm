...
type Horse = { id: string; name: string; notes: string | null; active: boolean };
type Chart = {
  id: string;
  horse_id: string;
  am_feed: string | null;
...
export default function BarnDisplay() {
    ...
    <div className="min-h-screen w-full bg-[#0a1a0a] text-white p-6 grid grid-rows-[auto_1fr_auto] gap-4 font-sans">
      ...
      <main className="grid grid-cols-[3fr_2fr] gap-6 min-h-0">
        ...
        <section className="rounded-2xl bg-black/40 border border-[#4ade80]/30 p-6 flex flex-col min-h-0">
          ...
          {horses.length === 0 ? (
            <div className="flex-1 grid place-items-center text-2xl text-white/50">
      ...
      <footer className="flex items-center justify-between px-2 text-white/50 text-sm">
        <span className="font-semibold text-[#4ade80]/80">BUCKS HAVEN FARM</span>
        <span>
          Last updated{" "}
          {lastUpdated
